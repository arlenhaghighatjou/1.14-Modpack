...
public class MagnetUpgradeWrapper extends UpgradeWrapperBase<MagnetUpgradeWrapper, MagnetUpgradeItem>
	...
	private boolean canFillBackpackWithXp() {
		return backpackWrapper.getFluidHandler().map(fluidHandler -> fluidHandler.fill(ModFluids.getExperienceTag(), 1, ModFluids.XP_STILL.get(), IFluidHandler.FluidAction.SIMULATE) > 0).orElse(false);
	}
	...
	private boolean tryToFillTank(ExperienceOrbEntity xpOrb, @Nullable LivingEntity entity, World world) {
		int amountToTransfer = XpHelper.experienceToLiquid(xpOrb.getXpValue());

		return backpackWrapper.getFluidHandler().map(fluidHandler -> {
			int amountAdded = fluidHandler.fill(ModFluids.getExperienceTag(), amountToTransfer, ModFluids.XP_STILL.get(), IFluidHandler.FluidAction.EXECUTE);
	...
	private int pickupItems(@Nullable LivingEntity entity, World world, BlockPos pos) {
		List<ItemEntity> itemEntities = world.getEntitiesWithinAABB(ItemEntity.class, new AxisAlignedBB(pos).grow(upgradeItem.getRadius()));
